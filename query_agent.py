import os
import glob
import sqlite3
import pandas as pd
from langchain_huggingface import HuggingFaceEndpoint
from db_init import ensure_database

# 1) Bootstrap DB from whatever CSVs are in data/
csv_files = glob.glob("data/*.csv")
ensure_database(csv_files, db_path="ecommerce.db")

# 2) LLM setup
os.environ["HUGGINGFACEHUB_API_TOKEN"] = os.getenv("HUGGINGFACEHUB_API_TOKEN", "")
llm = HuggingFaceEndpoint(
    repo_id="google/flan-t5-xl",
    temperature=0.0
)

def answer_query(question: str) -> tuple[str, pd.DataFrame]:
    """
    Returns (generated_sql, result_df)
    """
    # build the prompt
    prompt = (
        "You are an expert SQL generator.  "
        "Generate exactly one valid SQLite SQL query using ONLY the tables: "
        "total_sales, ad_sales, eligibility.  "
        "Output ONLY the SQL, no explanation.\n"
        f"Question: {question}\n"
        "SQL:"
    )

    # call the LLM
    resp = llm.generate([prompt])
    sql = resp.generations[0][0].text.strip().strip('"')

    # execute
    conn = sqlite3.connect("ecommerce.db")
    try:
        df = pd.read_sql_query(sql, conn)
    finally:
        conn.close()

    return sql, df
